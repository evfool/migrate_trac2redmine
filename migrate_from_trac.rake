  ...
  task :migrate_from_trac => :environment do
    ...
    module TracMigrate
      ...
      class TracMilestone < ActiveRecord::Base
        ...
        def due
          if read_attribute(:due) && read_attribute(:due) > 0
            Time.at(read_attribute(:due) / TIME_DIV).to_date
          else
            nil
          end
        end
        ...
        def completed
          if read_attribute(:completed) && read_attribute(:completed) > 0
            Time.at(read_attribute(:completed) / TIME_DIV).to_date
          else
            nil
          end
        end
      ...
      class TracAttachment < ActiveRecord::Base
        ...
        def time; Time.at(read_attribute(:time) / TIME_DIV) end
      ...
      class TracTicket < ActiveRecord::Base
        ...
        def time; Time.at(read_attribute(:time) / TIME_DIV) end
        def changetime; Time.at(read_attribute(:changetime) / TIME_DIV) end
      end
        ...
        if wiki.save
          TracWikiPage.order('name, version').all.each do |page|
            ...
            page.attachments.each do |attachment|
              next unless attachment.exist?
              next if p.attachments.find_by_filename(attachment.filename) #add only once per page
              attachment.open {
                a = Attachment.new :created_on => attachment.time
                a.file = attachment
                a.author = find_or_create_user(attachment.author)
